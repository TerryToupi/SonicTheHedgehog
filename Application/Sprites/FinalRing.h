...
class FinalRing : public scene::Sprite
{
public:
    FinalRing(int x, int y);
    ~FinalRing();
    ...
    // Callback for when collection animation finishes (GameScene handles the rest)
...
private:
    ...
    // Stage clear music
    sound::Track m_StageClearMusic = nullptr;

    static constexpr unsigned SPIN_START_FRAME = 0;
    static constexpr unsigned SPIN_END_FRAME = 3;
    static constexpr unsigned SPIN_DELAY_MS = 150;

    static constexpr unsigned COLLECTED_START_FRAME = 0;
    static constexpr unsigned COLLECTED_END_FRAME = 7;
    static constexpr unsigned COLLECTED_DELAY_MS = 120;
};
